...
type kind =
  ...
  | F of (string -> string list -> string) (* a function defines how to rearrange sub patches *)
  ...
  let str = match kind with
    | Seq ->
      let (off,str) = List.fold_left
          (fun (off,x) ((s,e),str) -> e, Printf.sprintf "%s%s%s" x (String.sub txt off (s - off)) str)
          (s, "") sub_str
      in
...
let mk_rewrite_patch ~loc ?(par=false) str = {
  loc;
  kind=F (fun _ _ -> str);
  sub=[];
  par;
...
let mk_seq_patch ~loc ?(par=false) sub = {
  loc;
  kind=Seq;
  sub;
  par;
...
let mk_get_patch ~loc arr_p index_p = {
  loc;
  kind=F (fun _ l -> match l with [a;i] -> Printf.sprintf "%s.!(%s)" a i | _ -> assert false);
  sub=[arr_p; index_p];
  par=false;
}
...
let mk_set_patch ~loc arr_p index_p val_p = {
  loc;
  kind=F (fun _ l -> match l with [a;i;v] -> Printf.sprintf "%s.!(%s) <- %s" a i v | _ -> assert false);
...
let mk_array_constr_patch ~loc ?(par=false) sub = {
  ...
  kind=F (fun txt -> function
      ...
      | _ ->
        ...
        patch_to_str txt {loc; kind=Seq; sub; par} |> Printf.sprintf "Floatarray.from_array %s"
    );
  sub;
  par;
}
