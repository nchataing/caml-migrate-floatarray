...
let rec rewrite_typ_var t =
  ...
  let desc =
    match t.desc with
    | Tvar _ ->
        let s = string_of_int !typ_var_n in
        incr typ_var_n;
        Tvar (Some ("_" ^ s))
...
let uni_trace_contains_escape =
  List.exists (fun x -> match x with Ctype.Unification_trace.Escape _ -> true | _ -> false)
  ...
  try
    ...
    with Ctype.Unify trace ->
      if uni_trace_contains_escape trace then
        let env =
          try Env.env_of_only_summary Envaux.env_from_summary env with Envaux.Error _ -> env
        in
        Ctype.moregeneral env false t_ref t_to_test
      else false
